...
function imagerot = rotateImageWithoutImrotate(image, degree)
%#codegen

assert(isa(image,'double') && all(size(image) <= [50 50]));
assert(isa(degree, 'double'));

coder.extrinsic('figure','subplot','clf','hold','imshow');
...
switch mod(degree, 360)
    ...
    otherwise
        ...
        % dest = minus(dest, min(dest)) + 1;
        dest(:,1) = minus(dest(:,1), min(dest(:,1))) + 1;
        dest(:,2) = minus(dest(:,2), min(dest(:,2))) + 1;
        ...
        for ii = 1:size(imagerot,1)
            for jj = 1:size(imagerot,2)
                ...
                if all(source >= 1) && all(source <= [m n])
                    ...
                    %cols = times(A, double(image(F(1):C(1),F(2):C(2),:)));
                    ceva = double(image(F(1):C(1),F(2):C(2),:)); % 3 x 3
                    cols = zeros(size(ceva));
                    for k=1:p
                        %cols(:,:,k) = times(A, ceva(:,:,k));
                        %cols(:,:,k) = A.*ceva(:,:,k);
                        for t=1:2
                            for s=1:2
                                cols(t,s,p) = A(t,s)*ceva(t,s,p);
                            end
                        end
                    end
