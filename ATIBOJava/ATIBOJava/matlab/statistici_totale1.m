function statistici_totale1(reffile, testdir)
...
plott=0;
%draw=0;
matr_global=NaN*ones(1,(length(ph_list)-2)/4);

number_of_letters = (length(ph_list)-2)/4;
%height_matr=[];
%width_matr=[];
...
%            cd(ph_list(i+3).name); %comentate pentru figura Bmare
            disp(ph_list(4*i+2).name)
%            dir_list=dir;
            %pause
            %keyboard 

%             letterModel=load(dir_list(6*i).name);  
%             letterModel=letterModel(:,1:2);
            %if new
...
%             disp('size de bitmapLetterModel:')
...
%            for j=2:length(dir_list)-2
                letter=load(ph_list(4*i+2).name);
                letter=letter(:,1:2);
...
%                 size(bitmapLetter)
                ...
                [Dist_hor, Dist_ver, ~, ~] = proiectie(bitmapLetterModel, bitmapLetter);
...
figure('name','Distantele globale','numbertitle','off')
plot(1,matr_global(1,:),'*')
hold on
errorbar(mean(matr_global),std(matr_global))


figure('name','Inaltimea literelor de test fata de litera model','numbertitle','off')
%height_matr
plot(height_matr)
hold on
...
figure('name','Latimea literelor de test fata de litera model','numbertitle','off')
plot(width_matr)
hold on
