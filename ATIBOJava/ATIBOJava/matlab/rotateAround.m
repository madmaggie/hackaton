...
assert(isa(pointY,'double'));
assert(isa(pointX,'double'));
assert(isa(angle,'double'));
%assert(isa(varargin,'double'));




%'imrotate' is not supported. Use a coder.extrinsic declaration 
% once in each function where ‘imrotate’ is used
coder.extrinsic('imrotate','padarray')
%y = imrotate(x,a);
...
[newX, newY] = pol2cart(theta+angle*180/pi, rho);
...
%rot = zeros(size(image)+[padX padY]);

% Rotate the image around the center.
%rot = imrotate(padded, angle, 'crop', method);
rot = anotherRotateImageWithoutImrotate(image,angle);
