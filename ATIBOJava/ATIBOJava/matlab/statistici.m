...
modelfile = '/media/micky/WORK/eclipseluna/ATIBOJava/LitereModel/Amic.txt';
testfile = '/media/micky/WORK/eclipseluna/ATIBOJava/matlab/testletters/0_raw.txt';

%[filename1, pathname] = uigetfile('*.TXT', 'Pick the prototype file');
%cd (pathname)
%letterModel=load([pathname filename1]);
letterModel = load(modelfile);
...
%cd(directoryname)
%ph_list=dir(directoryname);
%k=0;
%plott=0;

%matr_test=zeros(49,length(ph_list)-3);
%matr_model=zeros(length(ph_list)-3,4);
...
%           cd(ph_list(i).name);
            %dir_list=dir('/media/micky/WORK/eclipseluna/ATIBOJava/matlab/testletters/');
            %disp(dir_list(i+3).name)
            %letter=load(dir_list(i+3).name);
            letter = load('/media/micky/WORK/eclipseluna/ATIBOJava/matlab/testletters/0_raw.txt');
            letter=letter(:,1:2);
...
%             end
            ...
            [Dist_hor, Dist_ver, timelag_x, timelag_y] = proiectie(bitmapLetterModel, bitmapLetter)
            ...
            [Dist_rot, summodel, sumtest] = rotated_bitmap(bitmapLetterModel, bitmapLetter, centroids_model, centroids_test, 1)
            ...
            [Dist_u,D_u,k_u,w_u,rw_u,tw_u] = dtw_original(model1/360,test1/360,0)
