%function saveletterimage(filetoreadfrom, height, width, angle, filetosaveto) %#codegen

%assert(isa (filetoreadfrom, 'char*'))
%assert(isa (height,
...
folder = 'W:\eclipseluna\ATIBOJava\Bmare\Bmare__1424176974767';
filestoreadfrom = dir(folder);


for j=1:length(filestoreadfrom)
    if (not(filestoreadfrom(j).isdir) && (size(regexp(filestoreadfrom(j).name,'.txt'),1)>0) && (size(regexp(filestoreadfrom(j).name,'_raw.txt'),1)==0))
    ...
    if (not(filestoreadfrom(j).isdir) && (size(regexp(filestoreadfrom(j).name,'_raw.txt'),1)>0))
        ...
        clf
%         index = strfind(filetoreadfrom,'.txt')-1;
%         filetoreadfrom = strcat(filetoreadfrom(1:index),'_raw.txt');
        letterRaw = load([folder '\' filestoreadfrom(j).name]);
